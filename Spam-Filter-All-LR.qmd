...
df <- read.table("spambase.data", header = FALSE, sep = ",", col.names = names$V1)
```
...
### Eliminación de variables sin significancia estadistica según la Regresión logistica del df inicial

```{r}
df <- df |> dplyr:::select(-c(word_freq_make,word_freq_all,word_freq_3d,word_freq_mail,word_freq_receive,word_freq_will,word_freq_people,word_freq_report,word_freq_addresses, word_freq_email, word_freq_credit, word_freq_font, word_freq_lab, word_freq_labs, word_freq_telnet, word_freq_857, word_freq_415, word_freq_1999, word_freq_parts, word_freq_direct, word_freq_cs, word_freq_original, word_freq_table, char_freq_..1, char_freq_..2, char_freq_., capital_run_length_average))
```
...
```{r}
knn.pred=knn(train_knn,test_knn,train$Spam,k=5)
...
mod_dt2<-rpart(Spam~., train, 
          method = 'class',
          parms = list(  split = "information"),
          control = rpart.control(cp = 0.00078740,
                                  maxdepth = 7))
