...
ggplot(acc) + aes(x = k, y = accuracy) +geom_line(size = 0.5, colour = "#112446") +  theme_minimal() + geom_vline(xintercept = 6, color = "red")
```

```{r}
knn.pred=knn(train_knn,test_knn,train$Spam,k=6)
...
mod_dt2<-rpart(Spam~., train, 
          method = 'class',
          parms = list(  split = "information"),
          control = rpart.control(cp = 0.00039370,
                                  maxdepth = 7))
